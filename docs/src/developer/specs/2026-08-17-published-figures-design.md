...
detection-wider-than-validation shape docs spec §3.9's own near-miss detector uses. Neither
refusal is this gate's alone; §3.6 says why the blessing command makes both of them too.
...
It also removes a baseline no declaration claims, which is what stops a rename leaving behind
the orphan §3.5 reports — and is why it makes §3.5's two declaration refusals as well, before
it touches a file. A declaration that scan cannot read is a live baseline indistinguishable
from an orphan, so without them the command §3.5's near-miss advice sends a contributor to
would answer the near miss by deleting the pin, and report the deletion as a rename.
