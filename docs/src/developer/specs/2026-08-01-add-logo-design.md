...
which is all it has to do to choose between two artwork files.
...
The compositing itself is not `add_logo`'s: it lives in `plotting/_theme.canvas_rgb`,
because the isopleth label boxes ask the same question and take the answer unthresholded
