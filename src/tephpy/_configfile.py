...
#: one against the live API; the reference quadrant is out of scope for
#: ``tests/test_docs_snippets.py``, whose exclusion rests on a generated page
#: being unable to drift (configfile spec §3.6).
#:
#: Each names its file. ``save`` and ``load`` both default to the user
#: configuration directory, so an example that omitted the argument would read
#: or write the developer's own configuration when the gate executed it.
_REFERENCE_EXAMPLES: Final[Mapping[str, str]] = MappingProxyType(
    {
        "load": (
            "import tephpy\n"
            "\n"
...
def render_reference(config: Config) -> str:
    ...
    str
        ...
        ``_REFERENCE_METHODS``, each with the worked example
        ``_REFERENCE_EXAMPLES`` carries for it — the second rendering of the
        same tables the configuration template is rendered from, so a new
    ...
    for name in _REFERENCE_METHODS:
        ...
        lines.append("")
        lines.extend(
            f"      {line}".rstrip() for line in _REFERENCE_EXAMPLES[name].splitlines()
        )
        lines.append("")
    return "\n".join(lines)
