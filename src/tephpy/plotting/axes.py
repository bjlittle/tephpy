...
class TephigramAxes(Axes):
    ...
    def _set_view_from_bbox(
        ...
        direction: str = "in",
        mode: str | None = None,
        twinx: bool = False,  # noqa: FBT001 -- inherited matplotlib signature
        twiny: bool = False,  # noqa: FBT001 -- inherited matplotlib signature
    ) -> None:
        """Apply an unconstrained toolbar zoom without moving the axes box.

        Parameters
        ----------
        bbox : tuple of float
            Three- or four-value zoom request in display coordinates.
        direction : {"in", "out"}, optional
            Whether the rectangle describes the new view or an outward zoom.
        mode : {"x", "y"} or None, optional
            Optional single-axis constraint.
        twinx : bool, optional
            Whether an earlier axes already changed the shared x limits.
        twiny : bool, optional
            Whether an earlier axes already changed the shared y limits.
        """
        base = cast("Any", super())
        if mode is not None or twinx or twiny:
            base._set_view_from_bbox(  # noqa: SLF001 -- matplotlib override
                bbox, direction, mode, twinx, twiny
            )
            return

        prepared = cast("Any", self)._prepare_view_from_bbox(  # noqa: SLF001
            bbox,
            direction=direction,
            mode=mode,
            twinx=twinx,
        ...
        if prepared is None:
            return
        xbound, ybound = prepared
        old_x0, old_x1 = self.get_xbound()
        old_y0, old_y1 = self.get_ybound()
        x0, x1 = map(float, xbound)
        y0, y1 = map(float, ybound)
        spans = np.array([old_x1 - old_x0, old_y1 - old_y0, x1 - x0, y1 - y0])
        if not (np.isfinite(spans).all() and (spans > 0.0).all()):
            base._set_view_from_bbox(  # noqa: SLF001 -- matplotlib override
                bbox, direction, mode, twinx, twiny
            )
            return

        view_ratio = spans[1] / spans[0]
        target_yspan = spans[2] * view_ratio
        if target_yspan >= spans[3]:
            y_center = (y0 + y1) / 2.0
            y0, y1 = y_center - target_yspan / 2.0, y_center + target_yspan / 2.0
        else:
            target_xspan = spans[3] / view_ratio
            x_center = (x0 + x1) / 2.0
            x0, x1 = x_center - target_xspan / 2.0, x_center + target_xspan / 2.0
