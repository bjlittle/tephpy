...
`tephpy` plots and analyses tephigrams. Layered architecture: `sounding` (the
observed ascent) and `transforms` (T–ln θ math + Matplotlib projection) at the
base; `calc` and `io` build on `sounding`; `plotting` sits on top of all three.
Thermodynamics is delegated to MetPy; units are pint quantities.
...
- Docs follow Diátaxis; titles use CMOS headline style (`docs/src/developer/docs-style.rst`); see `docs/AGENTS.md`.
