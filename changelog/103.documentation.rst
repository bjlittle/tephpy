...
Sphinx's own ``extlinks_detect_hardcoded_links`` keep it that way. The pull-request
number towncrier appends to every changelog entry is now a link too. (:user:`claude`)
